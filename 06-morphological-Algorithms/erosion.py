...
# Checking if apply erosion
def apply_erosion(frame, structure):
    structure_counter = np.count_nonzero(structure == 1)
    count = 0
    ...
    for k in range(len(options)):
        ...
        for i in range(start, img_width - start):
            for j in range(start, img_height - start):
                ...
                result_image[i][j] = apply_erosion(frame, structuring_element)
                
        stacked_imgs = np.hstack((stacked_imgs, result_image))
    
    cv.imwrite(f"erosion_{img_name}{options}.jpg", stacked_imgs)
    print("Finish!")
